 ...
 *  Modified March 2009 by Jérôme Despatis and Jesse Tane for
 *  ATmega328 support
 *  Modified June 2009 by Michael Polli and Jesse Tane
 *  to fix a bug in setPeriod() which caused the timer to stop
 *  Modified Oct 2009 by Dan Clemens to work with timer3 
 *  of the ATMega1280 or Arduino Mega
 *
...
// printf("cycles %ld\n",cycles);
 ...
 {
  cycles = RESOLUTION - 1;
  clockSelectBits = _BV(CS12) | _BV(CS10); // out of bounds set max
 }
// printf("cycles %ld\n",cycles);
...
{
 TCCR3B &= ~(_BV(CS10) | _BV(CS11) | _BV(CS12)); // clear all clock sel bits
}
